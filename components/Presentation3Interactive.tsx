...
const Presentation3Interactive: React.FC = () => {
  ...
  return (
    <A3Page title="オプション" subtitle="お客様のご要望に合わせたオプションをご提案します">
      <div
      ...
      >
        ...
        <div className="relative h-[calc(100%-80px)] p-4">
          <div className="h-full flex flex-col">
            ...
            <div
              className="options-grid flex-1 mb-4"
              style={{
                display: 'grid',
            ...
            >
              {/* 外観① */}
              <div
                className="options-exterior-a"
                style={{ minWidth: 0, minHeight: 0, boxSizing: 'border-box' }}
              >
                <div
                  className={`h-full rounded-xl p-4 shadow-xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-gray-900/70 to-gray-800/50 border-red-600/30' : 'bg-white border-red-500/30'}`}
                >
                  <h4
                    className={`text-lg font-bold mb-3 text-center ${isDark ? 'text-red-400' : 'text-red-600'}`}
                  >
                    外観パターン①
                  </h4>
                  <div
                    className={`rounded-lg p-2 mb-4 aspect-[16/9] shadow-lg ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}
                  >
                    <div
                      className={`w-full h-full rounded flex items-center justify-center ${isDark ? 'bg-gradient-to-br from-gray-700 to-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-200'}`}
                    >
                      <span
                  ...
                  <div className="space-y-2">
                    {exteriorOption1.map((option) => (
                      <label
                        key={option.id}
                        className={`flex items-center gap-2 text-sm cursor-pointer p-2 rounded transition-colors ${isDark ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}
                      >
                        <input
                          type="checkbox"
                    ...
                    <div
                      className={`border-t-2 pt-3 mt-3 ${isDark ? 'border-red-600/50 bg-red-900/20' : 'border-red-500/50 bg-red-50'} p-3 rounded`}
                    >
              ...
              {/* 外観② */}
              <div
                className="options-exterior-b"
                style={{ minWidth: 0, minHeight: 0, boxSizing: 'border-box' }}
              >
                <div
                  className={`h-full rounded-xl p-4 shadow-xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-gray-900/70 to-gray-800/50 border-blue-600/30' : 'bg-white border-blue-500/30'}`}
                >
                  <h4
                    className={`text-lg font-bold mb-3 text-center ${isDark ? 'text-blue-400' : 'text-blue-600'}`}
                  >
                    外観パターン②
                  </h4>
                  <div
                  ...
                  <div className="space-y-2">
                    ...
                    <div
                      className={`border-t-2 pt-3 mt-3 ${isDark ? 'border-blue-600/50 bg-blue-900/20' : 'border-blue-500/50 bg-blue-50'} p-3 rounded`}
                    >
                      <div className="flex justify-between text-base font-bold">
                        <span className={isDark ? 'text-gray-300' : 'text-gray-700'}>小計</span>
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {['リビング', 'ダイニング', 'キッチン', '和室'].map((name, index) => (
                      <div
                        key={index}
                        className={`rounded-lg p-2 shadow-lg ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}
                      >
                        <div
                          className={`w-full aspect-[4/3] rounded flex items-center justify-center ${isDark ? 'bg-gradient-to-br from-gray-700 to-gray-900' : 'bg-gradient-to-br from-gray-50 to-gray-200'}`}
                        >
                          <span
                  ...
                  {/* 内装オプションリスト */}
                  <div
                    className={`flex-1 rounded-xl p-4 shadow-xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-gray-900/70 to-gray-800/50 border-gray-600/30' : 'bg-white border-gray-300'}`}
                  >
                    <div
                      className="interior-scroll print:max-h-none print:overflow-visible"
                      style={{
                    ...
                    >
                      <div className="space-y-1">
                        {interiorOptions.map((option) => (
                          <label
                            key={option.id}
                            className={`flex items-center gap-2 text-sm cursor-pointer p-2 rounded ${isDark ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}`}
                          >
                            <input
                              type="checkbox"
                              checked={option.checked}
                    ...
                    <div
                      className={`border-t-2 mt-4 pt-3 ${isDark ? 'border-gray-600 bg-gray-800/50' : 'border-gray-300 bg-gray-50'} p-3 rounded`}
                    >
                      <div className="flex justify-between text-base font-bold">
            ...
            <section className="options-total" style={{ marginTop: '24px' }}>
              <div
                className={`rounded-xl p-6 shadow-2xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-gray-900/90 to-black/80 border-red-600/50' : 'bg-gradient-to-br from-gray-50 to-white border-red-500/30'}`}
              >
                <h3
                ...
                <div className="grid grid-cols-2 gap-6">
                  {/* パターン① */}
                  <div
                    className={`rounded-xl p-5 shadow-xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-red-900/30 to-gray-900/50 border-red-600/50' : 'bg-gradient-to-br from-red-50 to-white border-red-400'}`}
                  >
                    <h4
                      className={`text-xl font-bold mb-2 text-center ${isDark ? 'text-red-400' : 'text-red-600'}`}
                    >
                      パターン①
                    </h4>
                    <p
                      className={`text-sm mb-3 text-center ${isDark ? 'text-gray-300' : 'text-gray-600'}`}
                    >
                      外観パターン① ＋ 内装オプション
                    </p>
                    <div className={`p-4 rounded-lg ${isDark ? 'bg-black/30' : 'bg-white'}`}>
                      ...
                      <div
                        className={`flex justify-between items-center p-3 rounded ${isDark ? 'bg-yellow-500/20' : 'bg-yellow-100'}`}
                      >
                        <span className={`text-sm ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                          月々
                        </span>
                        <span
                          className={`text-xl font-bold ${isDark ? 'text-yellow-400' : 'text-yellow-700'}`}
                        >
                  ...
                  {/* パターン② */}
                  <div
                    className={`rounded-xl p-5 shadow-xl border-2 print:overflow-visible ${isDark ? 'bg-gradient-to-br from-blue-900/30 to-gray-900/50 border-blue-600/50' : 'bg-gradient-to-br from-blue-50 to-white border-blue-400'}`}
                  >
                    <h4
                      className={`text-xl font-bold mb-2 text-center ${isDark ? 'text-blue-400' : 'text-blue-600'}`}
                    >
                      パターン②
                    </h4>
                    <p
